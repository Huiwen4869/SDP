...
<body>
    ...
    echo "<h1 style='color:black; margin-left:10%;margin-top:5%'> Feedbak Report</h1>";
    if ($result = mysqli_query($conn, $sql)) {
        if (mysqli_num_rows($result) > 0) {
            echo "<form action='search.php' method='POST'>";
            echo "<input type='text' name='search' style='float:right;margin-top:-10%;margin-right:30%;'>";
            echo "<input type='submit' name='searchbtn3' value='Search' style='float:right;margin-top:-10%;margin-right:23%;'>";
            echo "</form>";
            echo "<form action='include/fee.inc.php' method='POST'>";
